...
Declare
	...
	Cursor lc_tsr_backfill_data is
		select * from tsr_backfill where processed_fg is null and rownum < 100;
	...
	Loop
		...
		Begin
			...
			if ls_err_txt is null then
				...
				if ls_tsr_found_fg = 'Y' then
					...
					update tsr_backfill set error_txt = ls_err_txt, PROCESSED_FG = 'N', PROCESSED_DT = sysdate where PORT_BTL_SEID = lr_port_data.service_element_id;
					...
					--insert into tsr_backfil
						update tsr_backfill set TSR_BTL_SEID = ln_tsr_seid, error_txt = ls_err_txt, PROCESSED_FG = 'Y', PROCESSED_DT = sysdate where PORT_BTL_SEID = lr_port_data.service_element_id;
					else
						update tsr_backfill set TSR_BTL_SEID = ln_tsr_seid, error_txt = ls_err_txt, PROCESSED_FG = 'N', PROCESSED_DT = sysdate where PORT_BTL_SEID = lr_port_data.service_element_id;
					end if;
				end if;
			else
				update tsr_backfill set error_txt = ls_err_txt, PROCESSED_FG = 'N', PROCESSED_DT = sysdate where PORT_BTL_SEID = lr_tsr_backfill_data.PORT_BTL_SEID;
		...
		EXCEPTION
			WHEN OTHERS THEN
				...
				update tsr_backfill set error_txt = ls_err_txt,error_cd = ln_err_cd, PROCESSED_FG = 'N', PROCESSED_DT = sysdate where PORT_BTL_SEID = lr_port_data.service_element_id;
				commit;
