...
Declare
	ln_EQ_ATL_SEID			TSR_BACKFILL.EQ_ATL_SEID%type;
	ln_CPE_BTL_SEID   		TSR_BACKFILL.CPE_BTL_SEID%type;
	ln_IQ_ATL_SEID   		TSR_BACKFILL.IQ_ATL_SEID%type;
	ln_PORT_BTL_SEID  		TSR_BACKFILL.PORT_BTL_SEID%type;
	ln_TSR_BTL_SEID			TSR_BACKFILL.TSR_BTL_SEID%type;
	...
	Cursor lc_tsr_backfill_data is
		select * from tsr_backfill where PROCESSED_FG is null;
	...
	Loop
		...
		Begin
			...
			if ls_err_txt is null then
				...
				if ls_tsr_found_fg = 'Y' then
					...
					update tsr_backfill set error_txt = ls_err_txt, PROCESSED_FG = 'N', PROCESSED_DT = sysdate where PORT_BTL_SEID = lr_port_data.service_element_id and CPE_BTL_SEID = lr_cpe_data.service_element_id;
					...
					--insert into tsr_backfil
						update tsr_backfill set TSR_BTL_SEID = ln_tsr_seid, error_txt = ls_err_txt, PROCESSED_FG = 'Y', PROCESSED_DT = sysdate where PORT_BTL_SEID = lr_port_data.service_element_id and CPE_BTL_SEID = lr_cpe_data.service_element_id;
					--else
						--update tsr_backfill set TSR_BTL_SEID = ln_tsr_seid, error_txt = ls_err_txt, PROCESSED_FG = 'N', PROCESSED_DT = sysdate where PORT_BTL_SEID = lr_port_data.service_element_id and CPE_BTL_SEID = lr_cpe_data.service_element_id;
					--end if;
				end if;
			else
				update tsr_backfill set error_txt = ls_err_txt, PROCESSED_FG = 'N', PROCESSED_DT = sysdate where PORT_BTL_SEID = lr_tsr_backfill_data.PORT_BTL_SEID and CPE_BTL_SEID = lr_tsr_backfill_data.CPE_BTL_SEID;
		...
		EXCEPTION
			WHEN OTHERS THEN
				...
				update tsr_backfill set error_txt = ls_err_txt,error_cd = ln_err_cd, PROCESSED_FG = 'N', PROCESSED_DT = sysdate where PORT_BTL_SEID = lr_tsr_backfill_data.PORT_BTL_SEID and CPE_BTL_SEID = lr_tsr_backfill_data.CPE_BTL_SEID;
				commit;
