	...
	Loop
		...
		Begin
			...
			if ls_err_txt is null then
				...
				if ls_tsr_found_fg = 'Y' then
					...
					update tsr_backfill set error_txt = ls_err_txt, PROCESSED_FG = 'Y' where PORT_BTL_SEID = lr_port_data.service_element_id;
					...
					update tsr_backfill set TSR_BTL_SEID = ln_tsr_seid, error_txt = ls_err_txt, PROCESSED_FG = 'Y' where PORT_BTL_SEID = lr_port_data.service_element_id;
					
				end if;
			else
				update tsr_backfill set error_txt = ls_err_txt, PROCESSED_FG = 'Y' where PORT_BTL_SEID = lr_tsr_backfill_data.PORT_BTL_SEID;
		...
		EXCEPTION
			WHEN OTHERS THEN
				...
				update tsr_backfill set error_txt = ls_err_txt,error_cd = ln_err_cd, PROCESSED_FG = 'Y' where PORT_BTL_SEID = lr_port_data.service_element_id;
				commit;
